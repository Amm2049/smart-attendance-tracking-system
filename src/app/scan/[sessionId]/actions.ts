...
export async function submitAttendance(input: unknown): Promise<SubmitAttendanceResult> {
  const parsed = inputSchema.safeParse(input);
  if (!parsed.success) {
    return { ok: false, message: "Invalid check-in data. Please try again." };
  }
  ...
  try {
    ...
    const distanceM = haversineDistanceMeters({
      ...
      lat2: parsed.data.location.lat,
      lng2: parsed.data.location.lng,
    });
    ...
    if (
      existingStudent.name.trim().toLowerCase() !==
      parsed.data.name.trim().toLowerCase()
    ) {
      throw new Error(
        "Name does not match your student record. Please check your name and try again."
      );
    }
